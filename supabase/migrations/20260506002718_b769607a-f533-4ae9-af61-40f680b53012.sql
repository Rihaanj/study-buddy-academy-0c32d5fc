-- Safety: column default so naked inserts can't violate NOT NULL
ALTER TABLE public.daily_buff_usage ALTER COLUMN count SET DEFAULT 0;
...
BEGIN
  ...
  IF v_last IS NOT NULL AND v_last > (v_now - interval '5 seconds') THEN
    RAISE EXCEPTION 'Wait 5 seconds before activating another buff';
  END IF;

  -- COALESCE so SELECT INTO with no row gives 0 (was the source of NULL inserts)
  SELECT COALESCE(count, 0) INTO v_buffs_today
  FROM public.daily_buff_usage WHERE user_id = auth.uid() AND day = v_today;
  v_buffs_today := COALESCE(v_buffs_today, 0);
  ...
  IF COALESCE((v_meta->>'instant')::boolean, false) AND v_xp > 0 THEN
    ...
    v_xp_today := COALESCE(v_xp_today, 0);
    ...
    IF v_award <= 0 THEN
      ...
      INSERT INTO public.daily_buff_usage (user_id, day, count, updated_at)
        VALUES (auth.uid(), v_today, COALESCE(v_buffs_today,0) + 1, now())
        ON CONFLICT (user_id, day) DO UPDATE SET count = COALESCE(public.daily_buff_usage.count,0) + 1, updated_at = now();
    ...
    INSERT INTO public.daily_xp_progress (user_id, day, xp_gained, levels_gained, updated_at)
      ...
      ON CONFLICT (user_id, day) DO UPDATE SET
        xp_gained = COALESCE(public.daily_xp_progress.xp_gained,0) + v_award,
        levels_gained = COALESCE(public.daily_xp_progress.levels_gained,0) + GREATEST(0, v_new_level - v_old_level),
        updated_at = now();
    DELETE FROM public.inventory WHERE id = v_item.id;
    INSERT INTO public.daily_buff_usage (user_id, day, count, updated_at)
      VALUES (auth.uid(), v_today, COALESCE(v_buffs_today,0) + 1, now())
